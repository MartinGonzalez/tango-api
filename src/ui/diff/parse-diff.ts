...
/**
 * Parses unified diff output (e.g. from `git diff`) into structured DiffFile objects.
 */
...
function parseFileSection(section: string): DiffFile | null {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Skip lines starting with \ (No newline at end of file) and empty lines
  }
...
/** Count additions and deletions in a DiffFile. */
export function countFileChanges(file: DiffFile): { adds: number; dels: number } {
  let adds = 0;
  let dels = 0;
...
/**
 * Pairs delete/add lines for split (side-by-side) view rendering.
 * Context lines appear on both sides. Consecutive delete/add blocks are paired row-by-row.
 */
export function pairLinesForSplitView(
  lines: DiffLine[]
): [DiffLine | null, DiffLine | null][] {
