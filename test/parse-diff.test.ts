...
import { parseDiff, countFileChanges, pairLinesForSplitView } from "../src/ui/diff/parse-diff.ts";
...
describe("countFileChanges", () => {
  test("counts adds and deletes across hunks", () => {
    const result = countFileChanges({
      path: "test.ts",
      oldPath: null,
      status: "modified",
      isBinary: false,
      hunks: [
        {
          ...
          lines: [
            ...
            { type: "delete", content: "b", oldLineNo: 2, newLineNo: null },
            { type: "add", content: "B", oldLineNo: null, newLineNo: 2 },
            { type: "add", content: "B2", oldLineNo: null, newLineNo: 3 },
            { type: "context", content: "c", oldLineNo: 3, newLineNo: 4 },
          ],
        },
      ],
    });
    expect(result).toEqual({ adds: 2, dels: 1 });
  });

  test("returns zeros for empty hunks", () => {
    const result = countFileChanges({
      path: "empty.ts",
      oldPath: null,
      status: "modified",
    ...
    expect(result).toEqual({ adds: 0, dels: 0 });
  });
});

describe("pairLinesForSplitView", () => {
  test("pairs consecutive deletes and adds", () => {
    const pairs = pairLinesForSplitView([
      { type: "delete", content: "old1", oldLineNo: 1, newLineNo: null },
      { type: "delete", content: "old2", oldLineNo: 2, newLineNo: null },
      { type: "add", content: "new1", oldLineNo: null, newLineNo: 1 },
    ]);
    expect(pairs).toHaveLength(2);
    expect(pairs[0][0]?.content).toBe("old1");
    expect(pairs[0][1]?.content).toBe("new1");
    expect(pairs[1][0]?.content).toBe("old2");
    expect(pairs[1][1]).toBeNull();
  });

  test("context lines appear on both sides", () => {
    const pairs = pairLinesForSplitView([
      { type: "context", content: "same", oldLineNo: 1, newLineNo: 1 },
    ]);
    expect(pairs).toHaveLength(1);
    expect(pairs[0][0]).toBe(pairs[0][1]); // same reference
  });

  test("unmatched adds get null left side", () => {
    const pairs = pairLinesForSplitView([
      { type: "add", content: "new1", oldLineNo: null, newLineNo: 1 },
      { type: "add", content: "new2", oldLineNo: null, newLineNo: 2 },
    ]);
    expect(pairs).toHaveLength(2);
    expect(pairs[0][0]).toBeNull();
    expect(pairs[0][1]?.content).toBe("new1");
    expect(pairs[1][0]).toBeNull();
    expect(pairs[1][1]?.content).toBe("new2");
  });

  test("flushes remaining on end", () => {
    const pairs = pairLinesForSplitView([
      { type: "context", content: "a", oldLineNo: 1, newLineNo: 1 },
      { type: "delete", content: "b", oldLineNo: 2, newLineNo: null },
      { type: "add", content: "B", oldLineNo: null, newLineNo: 2 },
    ...
    expect(pairs).toHaveLength(2);
    expect(pairs[0][0]?.type).toBe("context");
    expect(pairs[1][0]?.content).toBe("b");
    expect(pairs[1][1]?.content).toBe("B");
  });
});
